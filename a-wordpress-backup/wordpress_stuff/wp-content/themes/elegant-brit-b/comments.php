...
<?php if ($comments) : ?>
<a id="comments"></a>
	<h3 id="comment-input"><?php comments_number('no comments', 'one comment', '% comments' );?> so far...</h3> 
...
<li id="comment-<?php comment_ID() ?>" <?php if($i&1) { echo 'class="odd"';} else {echo 'class="even"';} ?>>
  ...
  <span class="avatar"><?php 
   echo get_avatar( $comment, $size = '40', $default = '' ); 
   ?></span>
			<?php comment_author_link() ?> Said on <?php comment_date('F jS, Y') ?> at <?php comment_time() ?>: <?php edit_comment_link('e','',''); ?>
